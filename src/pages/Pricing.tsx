...
const Pricing = () => {
  ...
  const handlePlanSelection = async (selectedPlan: any) => {
    try {
      const {
        data: { user },
      ...
      if (!user) {
        // If user is not logged in, navigate to auth first
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      ...
      if (error) {
        ...
        // Use fallback plans if Supabase fetch fails
        setPlans(fallbackPlans);
      } else if (plansData && plansData.length > 0) {
        setPlans(plansData);
      } else {
        // Use fallback plans if no plans in database
        setPlans(fallbackPlans);
      }
      setLoading(false);
    };
