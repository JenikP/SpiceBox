        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-12">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">
              Start your weight loss journey with authentic Indian flavors. All plans include calorie-controlled meals, fresh ingredients, and the traditional spices you love.
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {plans.map(({ name, price, period, originalPrice, description, features, color, popular, bestValue }) => (
              <div
                key={name}
                className={`relative bg-white rounded-2xl shadow-xl p-8 transform hover:scale-105 transition-all duration-300 ${popular ? "ring-4 ring-red-500 ring-opacity-50" : ""}`}
              >
                {popular && (
                  <div className="absolute -top-4 left-4 bg-red-600 text-white px-3 py-1 rounded-full font-bold">Most Popular</div>
                )}
                {bestValue && (
                  <div className="absolute -top-4 right-4 bg-green-600 text-white px-3 py-1 rounded-full font-bold">Best Value</div>
                )}
                ...
                <div className="p-8 flex flex-col h-full">
                  <div className="text-center mb-8">
                    ...
                    <div className="flex items-center justify-center space-x-2 mb-2">
                      <span className={`text-5xl font-bold bg-gradient-to-r ${color} bg-clip-text text-transparent`}>{price}</span>
                      <div className="text-left">
                        <div className="text-gray-600 text-sm">{period}</div>
                        {originalPrice && <div className="text-gray-400 text-sm line-through">{originalPrice}</div>}
                      </div>
                    </div>
                    {originalPrice && (
                      <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
                        Save ${parseInt(originalPrice.slice(1)) - parseInt(price.slice(1))}
                      </div>
                    )}
                  </div>
                  ...
                  <Link to="/enter-details" className={`w-full block text-center py-4 px-6 rounded-xl font-semibold text-white bg-gradient-to-r ${color} hover:shadow-lg transform hover:scale-105 transition-all duration-200 mt-auto`}>
                    Select Plan
                  </Link>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {sellingPoints.map(({ iconColor, iconPath, title, text }) => (
                  <div key={title} className="text-center">
                    <div className={`w-16 h-16 bg-gradient-to-r ${iconColor} rounded-full flex items-center justify-center mx-auto mb-4`}>
          ...
          <div className="mt-16 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-red-500 rounded-2xl p-8 text-white">
              ...
              <p className="text-xl mb-6 opacity-90">
                Join thousands of Indian Australians who've already transformed their health with SpiceFit
              </p>
              <Link
                to="/enter-details"
